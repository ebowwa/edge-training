...
struct StreamingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Processor overlays
            ProcessorOverlayView(results: viewModel.processorResults)
            ...
            if showControls {
                VStack {
                    ...
                    HStack(spacing: 16) {
                        ...
                        // Processor toggles
                        ForEach(Array(viewModel.processors.enumerated()), id: \.offset) { index, processor in
                            ControlButton(
                                icon: processor.isEnabled ? "brain.head.profile.fill" : "brain.head.profile",
                                label: processor.name,
                                isActive: processor.isEnabled,
                                action: { viewModel.toggleProcessor(at: index) }
                            )
                        }
...
class ReceiverViewModel: ObservableObject {
    ...
    @Published var processorResults: [ProcessorResult] = []
    ...
    /// Registered frame processors
    var processors: [FrameProcessor] = [
        HTTPInferenceProcessor(),
        VisionKitProcessor()
    ]
    
    func toggleProcessor(at index: Int) {
        guard index < processors.count else { return }
        processors[index].isEnabled.toggle()
        if !processors.contains(where: { $0.isEnabled }) {
            processorResults = []
        }
    ...
    private func setupObservers() {
        ...
        Task { @MainActor in
            for await frame in relay.$receivedFrame.values {
                ...
                // Run enabled processors with throttling
                let enabledProcessors = self.processors.filter { $0.isEnabled }
                if !enabledProcessors.isEmpty && self.inferenceTask == nil {
                    let now = Date()
                    ...
                    self.inferenceTask = Task {
                        ...
                        for processor in enabledProcessors {
                            do {
                                let result = try await processor.process(frame)
                                results.append(result)
                            } catch {
                                print("[\(processor.name)] Error: \(error)")
                            }
                        }
                        ...
                        await MainActor.run {
                            self.processorResults = results
                            self.inferenceTimeMs = totalTime
